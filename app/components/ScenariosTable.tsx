    ...
    <div style={{ background: C.s, border: "1px solid " + C.b, borderRadius: 10, padding: 16 }}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
          <thead><tr>{["Price", "Perp P/L", "Hedge P/L", "Funding", "Total P/L", ""].map(function(hd) { return <th key={hd} style={{ textAlign: "right", padding: "5px 8px", borderBottom: "1px solid " + C.b, color: C.txM, fontWeight: 600, fontSize: 10, textTransform: "uppercase" }}>{hd}</th>; })}</tr></thead>
          <tbody>{tableRows.map(function(r, i) {
            return (<tr key={i} style={{ background: r.isLiq ? "rgba(255,96,144,0.06)" : "transparent" }}>
              ...
              <td style={{ textAlign: "right", padding: "3px 8px", color: r.fundingPL >= 0 ? C.g : C.r, fontSize: 10 }}>{r.fundingPL !== 0 ? (r.fundingPL >= 0 ? "+" : "") + r.fundingPL.toFixed(2) : "\u2014"}</td>
              <td style={{ textAlign: "right", padding: "3px 8px", fontWeight: 600, color: r.totalNetPL >= 0 ? C.g : C.r, background: r.totalNetPL > 0 ? "rgba(0,230,118," + Math.min(0.18, Math.abs(r.totalNetPL) / 5000) + ")" : r.totalNetPL < 0 ? "rgba(255,61,90," + Math.min(0.18, Math.abs(r.totalNetPL) / 5000) + ")" : "transparent", borderRadius: 3 }}>{r.totalNetPL >= 0 ? "+" : ""}{r.totalNetPL.toFixed(2)}</td>
